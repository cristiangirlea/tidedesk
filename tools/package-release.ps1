...
$notes = @(
    ...
    '',
    'Also experimental: connect by device ID. With a rendezvous service set in Host Settings and Viewer Settings (self-hosted; none is built in yet, see the rendezvous service guide), a viewer types the host''s device ID instead of anyone typing internet addresses, and headless hosts can be reached too. The service only introduces the two computers; the session still runs directly between them.',
    '',
    "[Running a rendezvous service](https://github.com/cristiangirlea/tidedesk/blob/v$Version/docs/rendezvous-server.md)",
    '',
