...
impl ViewerSettings {
    ...
    pub fn validate(&self) -> Result<()> {
        if !self.clipboard_shortcut.valid() || !self.mouse_shortcut.valid() {
            bail!("Shortcuts need Ctrl or Alt plus a letter or F1-F12.");
        }
        if self.clipboard_shortcut == self.mouse_shortcut {
            bail!("Clipboard and mouse shortcuts must be different.");
        }
        if self.clipboard_shortcut == settings_shortcut()
            || self.mouse_shortcut == settings_shortcut()
        {
