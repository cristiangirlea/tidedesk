...
struct RunningEguiAppState<EguiApp: App, EguiAppFactory: FnMut(Context) -> EguiApp> {
    ...
    /// Earliest delayed repaint egui asked for (TideDesk patch).
        ...
        if let Self::Running(state) = self {
            match event {
                UserEvent::RequestRepaint {
                    ...
                    {
                        // TideDesk patch: honour the requested delay instead of
                        // redrawing at once (a blinking text cursor otherwise
                        // spins a CPU core).
                        if when <= Instant::now() {
                            state.request_redraw();
                        } else {
                            state.repaint_at = Some(state.repaint_at.map_or(when, |t| t.min(when)));
                            event_loop.set_control_flow(ControlFlow::WaitUntil(
                                state.repaint_at.unwrap(),
                            ));
                        }
                    }
                }
    ...
    RunningEguiAppState<EguiApp, EguiAppFactory>
{
    ...
    pub(crate) fn handle_event(
        ...
        // TideDesk patch: keep a pending delayed repaint alive across events.
        match self.repaint_at {
            ...
            Some(t) => elwt.set_control_flow(ControlFlow::WaitUntil(t)),
        ...
        match window_event {
            WindowEvent::RedrawRequested => {
                // This pass re-requests any repaint it still needs.
                self.repaint_at = None;
